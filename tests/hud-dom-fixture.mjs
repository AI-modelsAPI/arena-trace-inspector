...
export function setupHud(sendMessage, autoEnabled=false, drawApi=undefined){
 ...
 let reloads=0;const location={pathname:'/agent/session-a',reload:()=>{reloads++;}};
 const sandbox={document,location,ArenaAutoDraw:drawApi,window:{innerWidth:1200,innerHeight:900,addEventListener:(name,fn)=>{events[name]=fn;},navigation:{addEventListener:(name,fn)=>{events[name]=fn;}}},MutationObserver:class{constructor(fn){hooks.repair=fn;}observe(){}},ArenaTracePanel:{create:(_parent,options)=>{hooks.panelOptions=options;return {render(view){hooks.view=view;},async renameModel(model,view){hooks.renames??=[];hooks.renames.push({model,sessionId:view.sessionId});}};}},chrome:{runtime:{id:'test-extension-id',onMessage:{addListener(fn){hooks.message=fn;}},sendMessage:msg=>{messages.push(msg);if(!sandbox.chrome.runtime.id)throw Error('Extension context invalidated.');if(msg.type==='ATI_AUTO_RENAME_GET')return Promise.resolve({enabled:autoEnabled});if(msg.type==='ATI_PULSE_GET')return Promise.resolve({pulse:null,fetchedAt:'2026-09-21T00:00:00Z',keys:[]});if(msg.type==='ATI_HUD_GET')return Promise.resolve({prefs:{collapsed:true,position:{x:.4,y:.4}}});if(msg.type==='ATI_HUD_SAVE')return Promise.resolve({prefs:msg.prefs});return sendMessage(msg);}}},setTimeout:(fn,delay)=>{const entry={fn,delay,cancelled:false};timers.push(entry);return entry;},clearTimeout:entry=>{if(entry)entry.cancelled=true;}};
 ...
 return {timers,events,messages,hooks,root,location,sandbox,invalidateContext(){sandbox.chrome.runtime.id=undefined;},get reloads(){return reloads;}};
}
